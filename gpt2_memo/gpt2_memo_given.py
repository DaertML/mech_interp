...
# --- FIXED Data & Metric Definition ---

# BETTER example that demonstrates memorization
CLEAN_TEXT = "By clicking register, you agree to, Etsy"
CORRUPTED_TEXT = "By choosing register, you agree to, Etsy"

TARGET_TOKEN_STR = " Etsy"

# --- FIXED calculate_target_position function ---

def calculate_target_position(model, text, target_token_str):
    """
    Calculates TARGET_POS_INDEX for a token that appears within the sentence.
    Returns the position where the model predicts the target token.
    """
    ...
    if not target_positions:
        raise ValueError(f"Target token '{target_token_str}' not found in text")
    
    if len(target_positions) > 1:
        print(f"Warning: Target token appears at multiple positions: {target_positions}")
        print("Using the last occurrence (most recent prediction).")
    
    # The TARGET_POS_INDEX is the position BEFORE the target token
    # This is where the model makes the prediction for the target token
    # Use the LAST occurrence for more interesting analysis
    target_pos_index = target_positions[-1] - 1
    
    print(f"\nTarget token '{target_token_str}' found at position(s): {target_positions}")
    print(f"Setting TARGET_POS_INDEX = {target_pos_index} (predicting token '{tokens[target_positions[-1]]}' after token '{tokens[target_pos_index]}')")
    
    return target_pos_index

# Calculate TARGET_POS_INDEX automatically
...
# --- Fix the get_token_loss function ---
...
def calculate_metric_restoration(
    ...
    was "restored" by the patch. 
    
    Handles cases where corrupted_loss < clean_loss by using absolute differences.
    """
    if corrupted_loss <= clean_loss:
        # If corrupted is already better than clean, restoration doesn't make sense
        # In this case, measure how much we made it worse
        if patched_loss >= corrupted_loss:
            return 0.0
        return (corrupted_loss - patched_loss) / (corrupted_loss + 1e-8)
    
    gap = corrupted_loss - clean_loss
    if gap == 0:
        return 0.0
    restoration = (corrupted_loss - patched_loss) / gap
    return max(0.0, min(1.0, restoration))  # Clamp between 0 and 1
...
def main():
    ...
    # Check if our memorization hypothesis makes sense
    print(f"\nLoss difference (corrupted - clean): {baseline_corrupted_loss - baseline_clean_loss:.3f}")
    
    if baseline_corrupted_loss <= baseline_clean_loss:
        print("\n*** ANALYSIS NOTE ***")
        print("Corrupted loss is not higher than clean loss.")
        print("This means the model finds the 'corrupted' version equally or more likely.")
        print("The analysis will still run, but interpret results carefully.")
        print("The model might not have strongly memorized this specific association.")
        print("Consider using a more distinctive example where the clean text is clearly memorized.")
