...
# The "memorized" text
CLEAN_TEXT = "Mr. and Mrs. Dursley, of number four, Privet Drive, were proud to say that they were perfectly normal, thank you very much."

# The "perturbed" text. "perfectly" and "completely" are both single tokens.
CORRUPTED_TEXT = "Mr. and Mrs. Dursley, of number four, PrivET Drive, were proud to say that they were completely normal, thank you very much."

# We want to measure the model's ability to predict "normal"
# The token "normal" is at index 20.
# We care about the model's prediction *at* index 19 (for the token "perfectly")
# The loss at position 19 is the neg log prob of the model predicting "normal"
TARGET_POS_INDEX = 19
...
TARGET_TOKEN_STR = " Etsy" # The token at index 20
...
def get_token_loss(
    ...
    """
    Calculates the negative log probability (loss) for the correct token
    at the specified target position.
    
    NOTE: The 'text' parameter is implicitly expected to be CLEAN_TEXT for
    ground truth, but we use the global CLEAN_TEXT and tokenize it here.
    """
    # Tokenize the CLEAN_TEXT string and move to the device for loss calculation.
    clean_tokens = model.to_tokens(CLEAN_TEXT).to(logits.device)
    
    # Use model's built-in loss_fn.
    # We pass the *clean* tokens to ensure we're always measuring the loss
    # against the *correct* memorized sequence.
    loss = model.loss_fn(logits, clean_tokens, per_token=True)
    return loss[0, target_pos_index].item()
...
def patch_hook_factory(
    ...
    def patch_hook(
        ...
        # Correctly index the 2D clean_act_tensor to get the feature vector 
        # at the specific position, and broadcast it to the batch dimension of 'act'.
...
def run_residual_stream_patching(
    ...
    for layer in range(n_layers):
        for pos in range(seq_len):
            ...
            patched_loss = get_token_loss(patched_logits, CLEAN_TEXT, TARGET_POS_INDEX)
...
def run_component_patching(
    ...
    mlp_patched_loss = get_token_loss(patched_logits_mlp, CLEAN_TEXT, TARGET_POS_INDEX)
    ...
    attn_patched_loss = get_token_loss(patched_logits_attn, CLEAN_TEXT, TARGET_POS_INDEX)
...
def run_neuron_ablation(
    ...
    ablated_loss = get_token_loss(ablated_logits, CLEAN_TEXT, TARGET_POS_INDEX)
...
def main():
    ...
    print(f"Target position index: {TARGET_POS_INDEX} (token: '{model.to_str_tokens(CLEAN_TEXT)[TARGET_POS_INDEX]}')")
    print(f"Prediction target: {TARGET_POS_INDEX+1} (token: '{TARGET_TOKEN_STR}')")

    
    # Get clean logits, cache, and baseline loss
    print("Running clean text...")
    # Use remove_batch_dim=True, which makes the cache tensors 2D [seq_len, d_features]
    clean_logits, clean_cache = model.run_with_cache(CLEAN_TEXT, remove_batch_dim=True)
    baseline_clean_loss = get_token_loss(clean_logits, CLEAN_TEXT, TARGET_POS_INDEX)
    print(f"Baseline Clean Loss:     {baseline_clean_loss:.3f}")
    ...
    baseline_corrupted_loss = get_token_loss(corrupted_logits, CLEAN_TEXT, TARGET_POS_INDEX)
    print(f"Baseline Corrupted Loss: {baseline_corrupted_loss:.3f}")
