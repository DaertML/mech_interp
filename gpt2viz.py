...
def global_induction_head_sweep():
    ...
    # The final 'X' (Query) should attend to the token following the first 'A' (which is 'B').
    INPUT_TEXT = "A B C D E F A X"
    
    # Output filename for the visualization
    OUTPUT_FILENAME = 'induction_head_heatmap.png'
    ...
    # Use split and join to clean up the tokens for display (removes leading spaces from BPE)
    tokens = [tokenizer.decode(t).strip() for t in input_ids]
    seq_len = len(tokens)
    
    # Dynamically find the indices of the critical tokens
    QUERY_TOKEN_IDX = seq_len - 1 # Index of 'X'
    
    # We look for the first 'A' that starts the pattern, excluding the last two tokens 
    # (which are the repeated 'A' and the query 'X')
    try:
        # Find the index of the token 'A' that precedes the repetition check
        # We need to find the token corresponding to the first occurrence of 'A'
        # The clean tokens array makes this search easier
        first_A_idx = tokens[:-2].index('A') 
        INDUCTION_TARGET_KEY_IDX = first_A_idx + 1 # Index of 'B'
    except ValueError:
        print("Error: The pattern 'A B ... A X' was not found or the sequence is not long enough.")
        return
    ...
    plt.figure(figsize=(12, 12)) # Increased size for better readability
    # Use 'viridis' for high contrast
    im = plt.imshow(induction_scores, cmap='magma', aspect='auto', interpolation='nearest') # Switched to magma
    ...
    plt.xticks(np.arange(num_heads), labels=[f"H{i}" for i in range(num_heads)], rotation=0)
    plt.yticks(np.arange(num_layers), labels=[f"L{i}" for i in range(num_layers)])
    
    plt.xlabel("Attention Head Index")
    plt.ylabel("Layer Index")
    
    plt.title(f"Induction Score Heatmap for GPT-2 Small\nQuery='{QUERY_TOKEN}' to Target='{TARGET_TOKEN}' (Max Score: {induction_scores.max():.4f})")
    
    # Add text annotations to the heatmap
    for i in range(num_layers):
        for j in range(num_heads):
            score = induction_scores[i, j]
            if score > 0.05: # Only annotate scores above a low threshold for clarity
    ...
    # --- CRITICAL FIX: Save the figure to a file ---
    plt.savefig(OUTPUT_FILENAME)
    print(f"\n[Visualization Saved] The heatmap has been saved to: {os.path.abspath(OUTPUT_FILENAME)}")
    
    # Call show last for interactive display if available
    plt.show()
