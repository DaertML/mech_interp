...
def load_and_analyze_custom_model():
    ...
    try:
        ...
        # Find the top 5 predicted tokens - FIXED APPROACH
        top_k = 1
        top_k_values, top_k_indices = torch.topk(next_token_logits, k=top_k)
        
        # FIX: Properly handle the top_k_indices tensor
        top_k_tokens = []
        for i in range(top_k_indices.shape[0]):
            idx = top_k_indices[i].item()  # Convert each tensor element to Python int
            top_k_tokens.append(model.tokenizer.decode(idx))

        # Find the source tokens that the final token (its) attended to most strongly
        attended_weights, attended_tokens_indices = torch.topk(attention_pattern, k=top_k)
        
        # FIX: Properly handle the attended_tokens_indices tensor
        attended_tokens = []
        for i in range(attended_tokens_indices.shape[0]):
            idx = attended_tokens_indices[i].item()  # Convert each tensor element to Python int
            attended_tokens.append(str_tokens[idx])
        ...
        for i, (token, weight) in enumerate(zip(attended_tokens, attended_weights)):
        ...
        # For better debugging, we print the specific type of the error as well
