...
# HookedTransformer.from_pretrained handles the config, weights, and tokenizer loading.
# The custom config (cfg) is no longer needed, as HookedTransformer infers it.
model = HookedTransformer.from_pretrained(
    MODEL_NAME, 
    device=device
)
# The old custom weight loading is removed:
...
# GPT-2 has 12 layers, so we'll analyze the attention from the first layer (index 0)
...
# Calculate the L2 norm of the W_O matrix for each head. GPT-2 has a standard W_O shape.
# W_O has shape [n_heads, d_model, d_head]
W_O_norm = torch.linalg.norm(model.blocks[0].attn.W_O, dim=1).detach().cpu().numpy() # [n_heads, d_head]
W_O_head_norm = W_O_norm.mean(axis=1) # Average norm across d_head to get a single scalar for each head: [n_heads]
...
X_FACTORS = 40
n_heads = model.cfg.n_heads # Get n_heads from the loaded model's config
